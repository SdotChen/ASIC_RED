...
**************************************************************************/
#define ENABLE_MATHUNIT 1
#define ENABLE_SHIFT (1-ENABLE_MATHUNIT)
    ...
    inout ingress_intrinsic_metadata_for_tm_t        ig_tm_md)
{
    ...
    table multicast_t {
	    actions = { multicast; }
		default_action = multicast(24);
	    size = 1;
    }

	action set_p4_header() {
		hdr.p4_header.seq_ig = 0;
		hdr.p4_header.seq_eg = 0;
		hdr.p4_header.tstamp_ig = ig_prsr_md.global_tstamp[31:0];
		hdr.p4_header.pad1 = 0;
		hdr.p4_header.qid = ig_tm_md.qid;
		hdr.p4_header.enq_qdepth = 0;
		hdr.p4_header.color = 0;
		hdr.p4_header.tstamp_eg = 0;
		hdr.p4_header.qdelta = 0;
	...
	Register<bit<32>,bit<9>>(512,0) reg_seq_ig;
	RegisterAction<bit<32>,bit<9>,bit<32>>(reg_seq_ig) _update_seq_ig =  {
	...
	action update_seq_ig() {
		hdr.p4_header.seq_ig = _update_seq_ig.execute(hdr.p4_header.egress_port);
	}

	@stage(1)
	table update_seq_ig_t {
		actions = { update_seq_ig;}
		default_action = update_seq_ig();
		size = 1;
	}
	...
	action get_seq_recirc() {
		hdr.p4_header.seq_recirc = _get_seq_recirc.execute(hdr.p4_header.egress_port);
	}
    ...
    apply {
        if(ig_intr_md.ingress_port == RECIRC_PORT) {
			drop_recirc_t.apply();
		} else if(ig_intr_md.ingress_port == 8 || ig_intr_md.ingress_port == 60) {
			multicast_t.apply(); // stage 0
			set_p4_header_t.apply(); // stage 1
			set_meter_t.apply(); // stage 1
			update_seq_ig_t.apply(); // stage 1
		}
    ...
    inout egress_intrinsic_metadata_for_output_port_t  eg_oport_md)
{
	...
	action mod_header() {
		hdr.p4_header.tstamp_eg = eg_prsr_md.global_tstamp[31:0];
		hdr.p4_header.enq_qdepth = (bit<32>)eg_intr_md.enq_qdepth;
		hdr.p4_header.qdelta = (bit<32>)eg_intr_md.deq_timedelta;
	}
	...
	Register<bit<32>,bit<9>>(512,0) reg_seq_eg;
	RegisterAction<bit<32>,bit<9>,bit<32>>(reg_seq_eg) _update_seq_eg =  {
		void apply(inout bit<32> reg_data, out bit<32> result) {
			result = reg_data;
	...
	action update_seq_eg() {
		hdr.p4_header.seq_eg = _update_seq_eg.execute(eg_intr_md.egress_port);
	}

	@stage(0)
	table update_seq_eg_t {
		actions = { update_seq_eg;}
		default_action = update_seq_eg();
		size = 1;
	}
	...
	RegisterAction<bit<32>,bit<9>,bit<32>>(reg_sum_qdepth) _get_and_set_sum_qdepth = {
		void apply(inout bit<32> reg_data, out bit<32> result) {
			result = reg_data;
			if(meta.option == 0) {
				reg_data = hdr.p4_header.enq_qdepth;
			} else if(meta.option == 2) {
				reg_data = reg_data + hdr.p4_header.enq_qdepth;
			}
		}
	};
...
#if ENABLE_SHIFT
	Register<bit<32>,bit<9>>(512) reg_sum_qdepth;
	RegisterAction<bit<32>,bit<9>,bit<32>>(reg_sum_qdepth) _add_sum_qdepth = {
		void apply(inout bit<32> reg_data) {
			reg_data = reg_data + hdr.p4_header.enq_qdepth;
		}
	};
	RegisterAction<bit<32>,bit<9>,bit<32>>(reg_sum_qdepth) _reset_sum_qdepth = {
		void apply(inout bit<32> reg_data,out bit<32> result) {
			result = reg_data;
			reg_data = 0;
		}
	...
	action add_sum_qdepth() {
		_add_sum_qdepth.execute(hdr.p4_header.egress_port);
	}

	@stage(2)
	table add_sum_qdepth_t {
		actions = {add_sum_qdepth;}
		default_action = add_sum_qdepth();
		size = 1;
	}

	action reset_sum_qdepth() {
		meta.sum_qdepth = _reset_sum_qdepth.execute(hdr.p4_header.egress_port);
	}

	@stage(2)
	table reset_sum_qdepth_t {
		actions = { reset_sum_qdepth;}
		default_action = reset_sum_qdepth();
		size = 1;
	}

	action get_aver_qdepth() {
		hdr.p4_header.enq_qdepth = meta.sum_qdepth >> POWER;
	}
	...
	Register<bit<16>,bit<9>>(512) reg_last_qdepth;
	RegisterAction<bit<16>,bit<9>,bit<16>>(reg_last_qdepth) _get_last_qdepth = {
		void apply(inout bit<16> reg_data, out bit<16> result) {
			result = reg_data;
			reg_data = hdr.p4_header.enq_qdepth[15:0];
		}
	};
	
	action get_last_qdepth() {
		meta.last_qdepth = _get_last_qdepth.execute(hdr.p4_header.egress_port);
	}

	@stage(4)
	table get_last_qdepth_t {
		actions = {get_last_qdepth;}
		default_action = get_last_qdepth();
		size = 1;
	}

	action get_weighted_qdepth() {
		meta.weighted_qdepth = hdr.p4_header.enq_qdepth[15:0] >> WQ;
	}
	...
	action get_weighted_last_qdepth() {
		meta.weighted_last_qdepth = meta.last_qdepth >> WQ;
	}

	@stage(5)
	table get_weighted_last_qdepth_t {
		actions = { get_weighted_last_qdepth;}
		default_action = get_weighted_last_qdepth();
		size = 1;
	}

	action minus_last_qdepth(){
		meta.last_qdepth = meta.last_qdepth - meta.weighted_last_qdepth;
	}

	@stage(6)
	table minus_last_qdepth_t {
		actions = {minus_last_qdepth;}
		default_action = minus_last_qdepth();
		size = 1;
	}

	action compute_aver_qdepth() {
		hdr.p4_header.aver_qdepth = meta.last_qdepth + meta.weighted_qdepth;
	}

	@stage(7)
	table compute_aver_qdepth_t {
		actions = { compute_aver_qdepth;}
		default_action = compute_aver_qdepth();
		size = 1;
	}
#endif

	action map_qdepth_to_prob(bit<10> prob){
		hdr.p4_header.drop_prob = (bit<16>)prob;
	}
...
#if ENABLE_SHIFT
		process_t.apply(); // stage 1
		if(meta.option == 0 && eg_intr_md.egress_port == RECIRC_PORT) {
			drop_t.apply(); // stage 2
		}
		if(meta.option == 2) {
			add_sum_qdepth_t.apply(); // stage 2
		} else if(meta.option == 1) {
			reset_sum_qdepth_t.apply(); // stage 2
			get_aver_qdepth_t.apply(); // stage 3
			get_last_qdepth_t.apply(); // stage 4
			get_weighted_qdepth_t.apply(); // stage 4
			get_weighted_last_qdepth_t.apply(); // stage 5
			minus_last_qdepth_t.apply(); // stage 6
			compute_aver_qdepth_t.apply(); // stage 7
			map_qdepth_to_prob_t.apply(); //stage 8
		}
#endif
	}
